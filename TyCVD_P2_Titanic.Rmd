...
```{r integracion_y_seleccion, size="small",echo=FALSE, warning=FALSE}
...
Partimos el campo ticket en prefijo y número de ticket.
A partir del numero de ticket obtenemos grupos en función de la cantidad de numero del número de ticket y vemos como se relaciona con sobrevivir.
Vemos como se relacionan los prefijos del ticket con sobrevivir.
Obtenemos una variable que nos indica la catidad de números de tickets(TicketFreq) iguales y vemos su relacion con sobrevivir. Hemos tenido que corregir 4 casos de valores perdidos en nuestra nueva variable debido a que estos casos no tenian número de ticket.  

  
  
```{r pruebas_mrzs_Ticket,fig.width=7, fig.height=4, size="small",echo=FALSE, warning=FALSE}
#TRABAJO CON TICKET
muestra$TktNum <- NA
...
1. Agrupamos los titulos poco frecuentes en tres grupos según su probabilidad de supervivencia, los que todas las personas con ese título han muerto van a 'titulogrupo0', si han sobrevivido todos a 'titulogrupo1' y si la probabilidad es diferente al grupo 'titulogrupo05'. Comprovamos que la relación de los grupos con supervivencia.

2. Creamos una nueva variable llamada FamSize que nos indica el número de familiares a bordo, esto se obtiene con 'SibSp + Parch + 1'. Mostramos gáficamente la relación con supervivencia. 
...
4. Vamos a agrupar las familias para ver exactamente cuantos familiares viajaban juntos. Esto lo haremos concatenando el Apellido con el número de familiars y el número de ticket. Hemos añadido el número de ticket porque si solo juntamos apellido y número de familiares se mezclaban familias que coincidían en apellido y numero de familiares pero viajaban con diferente ticket. Tras agruparlos correctamente creamos grupos genéricos según el número real de familiares que viajaban juntos.
Finalmente visualizamos la relación de nuestra nueva variable 'FamilyIDTKGrouped' con la supervivencia.

```{r pruebas_mrzs_Name,fig.width=7, fig.height=4, size="small", echo=FALSE, warning=FALSE}
#TRABAJO CON NAME
...
FamilyIDTKGrouped[FamSize == 2] <- 'Size2'
#mostramos frecuencia de los datos
...
FamilyIDTKGrouped[FamilyIDTKGrouped %in% Smalls$FamilyIDTKGrouped] <- 'Size2'
#mostramos frecuencia de los datos
#table(FamilyIDTKGrouped)
...
#VAMOS A AGRUPAR EN FUNCION DE MIEMBROS
# agrupamos las familias que deberían tener 3 o más familiares pero en el recuento no aparecen.
famIDs <- data.frame(table(FamilyIDTKGrouped))
Alones <- famIDs[famIDs$Freq == 3,]
FamilyIDTKGrouped[FamilyIDTKGrouped %in% Alones$FamilyIDTKGrouped] <- 'Size3'
Smalls <- famIDs[famIDs$Freq == 4,]
FamilyIDTKGrouped[FamilyIDTKGrouped %in% Smalls$FamilyIDTKGrouped] <- 'Size4'
Alones <- famIDs[famIDs$Freq == 5,]
FamilyIDTKGrouped[FamilyIDTKGrouped %in% Alones$FamilyIDTKGrouped] <- 'Size5'
Smalls <- famIDs[famIDs$Freq == 6,]
FamilyIDTKGrouped[FamilyIDTKGrouped %in% Smalls$FamilyIDTKGrouped] <- 'Size6'
Alones <- famIDs[famIDs$Freq == 7,]
FamilyIDTKGrouped[FamilyIDTKGrouped %in% Alones$FamilyIDTKGrouped] <- 'Size7'
Smalls <- famIDs[famIDs$Freq >= 8 & famIDs$Freq <= 11,]
FamilyIDTKGrouped[FamilyIDTKGrouped %in% Smalls$FamilyIDTKGrouped] <- 'SizeBig'
#mostramos frecuencia de los datos
#table(FamilyIDTKGrouped)
#vemos proporcion de supervivencia de las familias
#aggregate(muestra$Survived ~ FamilyIDTKGrouped, FUN=function(x) {sum(x)/length(x)})
...
Creamos una nueva variable que nos indica si la persona es una madre o no. Esta variable se obtiene de las observacviones que cumplen la condicion de tener el título 'Mrs' y que la variable Parch que indica el número de hijos o padres sea mayo que 0.
Visualizamos la relación de nuestra nueva variable 'IsMother' con la supervivencia.


```{r pruebas_mrzs_Parch,fig.width=7, fig.height=4, size="small",echo=FALSE, warning=FALSE}
...
En esta variable nos encontramos varios valores perdidos que sustituimos por el valor más frecuente.
Visualizamos la relación de Embarqued con la supervivencia.

```{r pruebas_mrzs_Embarked,fig.width=7, fig.height=4, size="small",echo=FALSE, warning=FALSE}
#TRABAJO CON Embarqued
...
Existen demasiados valores perdidos y no tenemos una forma buena de predecirlos, sin que produzacan demasiado ruido, por lo que vamos a ignorar esta variable.

### Trabajo con Fare

Existen varios valores perdidos que sustituimos por la mediana del 'Fare' en función de la clase, el sexo y el número de hijos. 
También existen ciertos pasajeros con Fare 0, pero los vamos a dejar como están.  
  
Vemos el gráfico de caja de la variable y observamos que exiten valores extremos que pueden afectar a la variable para solucionar esto vamos agrupamos Fare en categorías y añadimos los valores extremos a la última categoría. La agrupación se crea en la variable 'FareGroups' y visualizamos la relación de nuestras categorías con la supervivencia.  
...
Como idea feliz vamos a probar a juntar las variables de Fare y sex, ya que hay diferencias significativas en la supervivencia de hombres y mujeres. 
Creamos una nueva variable 'FareIndividualBySex' que incrementa el Fare de las mujeres en 60, con esto conseguimos paliar un poco los efectos de los valores extremos, pero tendremos que analizar más adelante si es útil o no. También creamos una variable(FareIndividualBySexGroups) que agrupe estos nuevos Fares y visualizamos la relación de nuestros grupos con la supervivencia.


```{r pruebas_mrzs_Fare,fig.width=7, fig.height=4, size="small",echo=FALSE, warning=FALSE}
#TRABAJO CON Fare
...
boxplot(muestra[muestra$Fare!=0,]$Fare,
        data = muestra,
        ylab = "Fare del pasajero",
        main = "grafico de caja de Fare sin los ceros")
#boxplot.stats(muestra$Fare)$out
...
#vamos a probar a crear una variable que sume el fare individual + el sexo * 60
...
boxplot(FareIndividualBySex,
        data = muestra,
        ylab = "FareIndividualBySex del pasajero",
...
Mostramos el Histograma de  la variable y vemos el número de agrupaciónes óptimo para agrupar la variable. Creamos la variable 'ageGroups' con los grupos.
...
- El primer método de imputación será imputar por la edar media del titulo de la persona. Creamos la variable Ages.
...
- El tercer método es por regresión. Primero vemos como se ditribuye la edad visualizando su histograma con un mayor número de divisiones. Representamos la relación entre las variables cuantitativas y también mostramos la relación entre las variables cualitativas y la edad.
Generamos un modelo para predecir el logaritmo de la edad(esto mejora la falta de normalidad de la variable Age) a partir de las variables Pclass, SibSp, Parch, Fare, Sex, Embarked, IsMother, LoneWolfs, FamSize, Titulo y usamos stepAIC para que seleccione el modelo que mejor funciona, partir de esas variables se va quedando con las variables más representativas. Visualizamos los gráficos de los residuos del modelo y creamos la variable RegresionAges para almacenar las edades imputadas.
...
```{r pruebas_mrzs_Edad,fig.width=7, fig.height=4, size="small", echo=FALSE, warning=FALSE}
#TRABAJO CON EDAD
par(mfrow=c(1,1))
...
muestra$youngTravelAl <- muestra$Age < 16 & muestra$Parch == 0
#visualizacion grafica
...
### Trabajo con Pclass
Pclas está como integer pero realmente es una categórica, podemos crear una nueva varable tipo factor PclassFactor

```{r pruebas_mrzs_Pclass,fig.width=7, fig.height=4, size="small", echo=FALSE, warning=FALSE}

#TRABAJO CON PCLASS
#vamos a crear una variable con pclass factorizada

muestra$PclassFactor<-as.factor(muestra$Pclass)
#vemos proporcion de supervivencia de los grupos por sexo
#aggregate(muestra$Survived ~ muestra$Pclass+muestra$Sex, FUN=function(x) {sum(x)/length(x)})
#visualizacion grafica
...
En la variable Ticket:
Hemos tenido que corregir 4 casos de valores perdidos en nuestra nueva variable debido a que estos casos no tenian número de ticket y solo aparecía la palabra LINE. Como solo nos interesa para una variable que agrupa los tamaños de número de ticket le asignamos un 0 en dicha variable, en la variable original no hacemos nada porque no va a ser utilizada.

En la bariable Embarked:
Nos encontramos varios valores perdidos que sustituimos por el valor más frecuente.

En la variable Cabina:
Existen demasiados valores perdidos y no tenemos una forma buena de predecirlos, sin que produzacan demasiado ruido, por lo que vamos a ignorar esta variable.

En la variable Fare:
Existen ciertos pasajeros con Fare 0, pero los vamos a dejar como están.

En la variable Age:
Limpiamos los valores perdidos, esto lo vamos a hacer de varias maneras y luego estudiaremos cual es la mejor:
- El primer método de imputación será imputar por la edar media del titulo de la persona. Creamos la variable Ages.
- El segundo método es KNN con el que imputamos los valores perdidos de Age en función de las variables "Pclass", "Sex", "SibSp", "Parch", "Fare", "Embarked", "Titulo", 'FamSize'. Creamos la variable KNNAges
- El tercer método es por regresión. Primero vemos como se ditribuye la edad visualizando su histograma con un mayor número de divisiones. Representamos la relación entre las variables cuantitativas y también mostramos la relación entre las variables cualitativas y la edad.
Generamos un modelo para predecir el logaritmo de la edad(esto mejora la falta de normalidad de la variable Age) a partir de las variables Pclass, SibSp, Parch, Fare, Sex, Embarked, IsMother, LoneWolfs, FamSize, Titulo y usamos stepAIC para que seleccione el modelo que mejor funciona, partir de esas variables se va quedando con las variables más representativas. Visualizamos los gráficos de los residuos del modelo y creamos la variable RegresionAges para almacenar las edades imputadas.
...
```{r pruebas_conversion, size="small", echo=FALSE, warning=FALSE}
...
#Convertimos las variables char, logicas y numericas cualitalivas a factor
muestra$SurvivedFactor<-as.factor(muestra$Survived)
muestra$FamilyIDTKGrouped<-as.factor(muestra$FamilyIDTKGrouped)
muestra <- muestra %>% mutate_if(is.logical, as.factor)
...
# las dummys quedan guardadas en muestrascaledumy, 
#si alguna variable dummy es interesante podemos agregarla a la muestra
...
En la variable Fare:
Para solucionar el problema de los valores extremos vamos agrupamos Fare en categorías y añadimos los valores extremos a la última categoría.

En la variable Age:
Para solucionar el problema de los valores extremos agrupamos las edades e incluimos a los valores extremos en la última categoría. 

# 4. Análisis de los datos.
## 4.1. Selección de los grupos de datos que se quieren analizar/comparar (planificación de los análisis a aplicar).

Inicialmente ponemos las varaiables en grupos en función de la relación que tienen por como se han creado:
...
Vamos a analizar la asociacion entre variables de nuestros grupos y entre las variables y Survived:

Mejor relacion con survived por grupos:  
"FareIndividualBySex","FareIndividualBySexGroups","FareIndividual"  
"IsMother","TicketFreq"  
"LoneWolfs","IsMother"  
"TicketFreq"  
"LoneWolfs","ageGroupsByCut","Age"  
"FareIndividualBySex","FareIndividualBySexGroups","Sex"  

Grupos de variables interesantes tras este análisis:
"FareIndividualBySex","IsMother","LoneWolfs","TicketFreq","ageGroupsByCut","Embarked","Pclass"  
"FareIndividualBySexGroups","IsMother","LoneWolfs","TicketFreq","ageGroupsByCut","Embarked","Pclass",'FarePw2'  
"FamilyIDTKGrouped","Titulo","FareIndividual","IsMother","LoneWolfs","youngTravelAl","Age","Embarked","Pclass"  


Mostrámos gráficamente las relaciones existentes

```{r pruebas_analisis_filtros,fig.width=7, fig.height=4, size="small", echo=FALSE, warning=FALSE}
...
# print('Mostramos matriz de correlaciones entre las variables numéricas independientes')
# auxmat <- cov2cor(cov.wt(cbind(muestra$Pclass,muestra$Age,muestra$SibSp,muestra$Parch,muestra$Fare,muestra$tamanoticket,muestra$TicketFreq,muestra$FamSize,muestra$FareIndividual,muestra$Ages,muestra$KNNAges,muestra$RegresionAges,as.numeric(muestra$FareIndividualBySex),as.numeric(muestra$FamilyIDTKGrouped)))$cov)
...
# corrplot(cor(dfaux), method="number", is.corr=FALSE)
# 
# #Mostramos matriz de correlaciones y gráfico entre las variablesDE GRUPOS
# dfaux<-muestra[,c("Fare","FareGroups","FareIndividual","FareIndGroups","FareIndividualBySex","FareIndividualBySexGroups",'FarePw2','FarePw3')]
# dfaux <- dfaux %>% mutate_if(is.factor, as.numeric)
# #cor(dfaux)
# corrplot(cor(dfaux), method="number", is.corr=FALSE)
# 
# #Mostramos matriz de correlaciones y gráfico entre las variablesDE GRUPOS
# dfaux<-muestra[,c("Titulo","FamilyIDTKGrouped","FamSize","TicketFreq","IsMother")]
# dfaux <- dfaux %>% mutate_if(is.factor, as.numeric)
# cor(dfaux)
# corrplot(cor(dfaux), method="number", is.corr=FALSE)
# 
# #Mostramos matriz de correlaciones y gráfico entre las variablesDE GRUPOS
# dfaux<-muestra[,c("SibSp","Parch","FamSize","IsMother","LoneWolfs","FamilyIDTKGrouped","youngTravelAl")]
# dfaux <- dfaux %>% mutate_if(is.factor, as.numeric)
# cor(dfaux)
# corrplot(cor(dfaux), method="number", is.corr=FALSE)
# 
# #Mostramos matriz de correlaciones y gráfico entre las variablesDE GRUPOS
# dfaux<-muestra[,c("TicketFreq","tamanoticket","FamilyIDTKGrouped")]
# dfaux <- dfaux %>% mutate_if(is.factor, as.numeric)
# cor(dfaux)
# corrplot(cor(dfaux), method="number", is.corr=FALSE)
# 
# #Mostramos matriz de correlaciones y gráfico entre las variablesDE GRUPOS
# dfaux<-muestra[,c("Age","ageGroupsByCut","Ages","KNNAges","RegresionAges","LoneWolfs",'AgePw2','AgePw3',"youngTravelAl")]
# dfaux <- dfaux %>% mutate_if(is.factor, as.numeric)
# cor(dfaux)
# corrplot(cor(dfaux), method="number", is.corr=FALSE)
# 
# #Mostramos matriz de correlaciones y gráfico entre las variablesDE GRUPOS
# dfaux<-muestra[,c("Sex","FareIndividualBySex","FareIndividualBySexGroups")]
# dfaux <- dfaux %>% mutate_if(is.factor, as.numeric)
# cor(dfaux)
# corrplot(cor(dfaux), method="number", is.corr=FALSE)


#RELACIONES CON SURVIVED
...
#Mostramos matriz de correlaciones y gráfico entre las variablesDE GRUPOS
print('Mostramos matriz de correlaciones entre las variables del grupo1')
dfaux<-muestra[1:891,c("Survived","Fare","FareGroups","FareIndividual","FareIndGroups","FareIndividualBySex","FareIndividualBySexGroups",'FarePw2','FarePw3')]
dfaux <- dfaux %>% mutate_if(is.factor, as.numeric)
#cor(dfaux)
corrplot(cor(dfaux), method="number", is.corr=FALSE)

#Mostramos matriz de correlaciones y gráfico entre las variablesDE GRUPOS
print('Mostramos matriz de correlaciones entre las variables del grupo2')
dfaux<-muestra[1:891,c("Survived","Titulo","FamilyIDTKGrouped","FamSize","TicketFreq","IsMother")]
dfaux <- dfaux %>% mutate_if(is.factor, as.numeric)
#cor(dfaux)
corrplot(cor(dfaux), method="number", is.corr=FALSE)

#Mostramos matriz de correlaciones y gráfico entre las variablesDE GRUPOS
print('Mostramos matriz de correlaciones entre las variables del grupo3')
dfaux<-muestra[1:891,c("Survived","SibSp","Parch","FamSize","IsMother","LoneWolfs","FamilyIDTKGrouped","youngTravelAl")]
dfaux <- dfaux %>% mutate_if(is.factor, as.numeric)
#cor(dfaux)
corrplot(cor(dfaux), method="number", is.corr=FALSE)

#Mostramos matriz de correlaciones y gráfico entre las variablesDE GRUPOS
print('Mostramos matriz de correlaciones entre las variables del grupo4')
dfaux<-muestra[1:891,c("Survived","TicketFreq","tamanoticket","FamilyIDTKGrouped")]
dfaux <- dfaux %>% mutate_if(is.factor, as.numeric)
#cor(dfaux)
corrplot(cor(dfaux), method="number", is.corr=FALSE)

#Mostramos matriz de correlaciones y gráfico entre las variablesDE GRUPOS
print('Mostramos matriz de correlaciones entre las variables del grupo5')
dfaux<-muestra[1:891,c("Survived","Age","ageGroupsByCut","Ages","KNNAges","RegresionAges","LoneWolfs",'AgePw2','AgePw3',"youngTravelAl")]
dfaux <- dfaux %>% mutate_if(is.factor, as.numeric)
#cor(dfaux)
corrplot(cor(dfaux), method="number", is.corr=FALSE)

#Mostramos matriz de correlaciones y gráfico entre las variablesDE GRUPOS
print('Mostramos matriz de correlaciones entre las variables del grupo6')
dfaux<-muestra[1:891,c("Survived","Sex","FareIndividualBySex","FareIndividualBySexGroups")]
dfaux <- dfaux %>% mutate_if(is.factor, as.numeric)
#cor(dfaux)
corrplot(cor(dfaux), method="number", is.corr=FALSE)
...
El análisis de factores es similar al análisis de componetes principales, le aportamos un conjunto de variables y construye un nuevo conjunto de datos con nuevas variables que representan a los datos originales.  
Los conjuntos de datos obtenidos con este sistema no nos han dado buenos resultados.

```{r pruebas_analisis_factores, size="small", echo=FALSE, warning=FALSE}
#resumen estadístico de las variables del dataset sería un análisis estadístico
#summary(muestra)
...
noNaGroup<-muestra[,c("FareIndividualBySex","IsMother","LoneWolfs","TicketFreq","ageGroupsByCut","Embarked","Pclass")] # 100% con 11 componentes
#noNaGroup<-muestra[,c("FareIndividualBySexGroups","IsMother","LoneWolfs","TicketFreq","ageGroupsByCut","Embarked","Pclass",'FarePw2')] # 100% con 18 componentes
#noNaGroup<-muestra[,c("FamilyIDTKGrouped","Titulo","FareIndividual","IsMother","LoneWolfs","youngTravelAl","Age","Embarked","Pclass")]
...
res.famd <- FAMD(noNaGroup, ncp = 11,graph = F)
...
trainFAMD <- data.frame(muestra[,1:2],res.famd$ind$coord[1:nrow(muestra),1:11])
...
```{r pruebas_analisis_wrapper_methods, size="small", echo=FALSE, include=FALSE}
...
trainAIC <- muestra[1:891,c("Survived","Embarked","Titulo","FareIndGroups","FareIndividualBySexGroups","ageGroupsByCut","PclassFactor","FamilyIDTKGrouped","AgeScaled","SibSpScaled","ParchScaled","tamanoticketScaled","TicketFreqScaled","FareIndividualScaled","FareIndividualBySexScaled","LoneWolfsDummy","IsMotherDummy","youngTravelAlDummy")] #85%
testAIC <- muestra[892:1309,c(4,9,11,13,14,20)]
trainAIC2 <- muestra[1:891,c("Survived","FareIndividualBySex","IsMother","LoneWolfs","TicketFreq","ageGroupsByCut","Embarked","Pclass")] #80.4
testAIC2 <- muestra[892:1309,c(4,9,11,13,14,20)]
...
# trainFamdBKpAIC <- BkpTrainFamd11c100[1:891,]
# testFamdBKpAIC <- BkpTrainFamd11c100[892:1309,]
trainAIC3 <- muestra[1:891,c("Survived","FareIndividualBySexScaled","IsMother","LoneWolfs","TicketFreqScaled","PclassScaled")]
trainAIC3 <- cbind(trainAIC3,dmmyEmbarked[1:891,], dmmyageGroupsByCut[1:891,])#80.8

trainAIC$Survived<-as.factor(trainAIC$Survived)
...
trainAIC$FamilyIDTKGrouped <- relevel(trainAIC$FamilyIDTKGrouped, ref = 'Alone')
...
#mymod1<-glm(Survived ~ Titulo+FareIndividualBySexScaled+FamilyIDTKGrouped, data = trainAIC ,binomial) #83.3% es la seleccion optima de rlm
#mymod1<-glm(Survived ~ Titulo+FareIndividual+Pclass+FamilyIDTKGrouped, data = trainRlm ,binomial) #85% es la seleccion optima de rlm
...
#mymod2 <- glm(Survived ~ ., data = trainFamdBKpAIC, binomial)#80.5
#mymod1<-mymod2
...
Variables recomendadas por este método:  
"Pclass", "FareIndividualBySex", "ageGroupsByCut"  
"Pclass", "Titulo", "FamSize", "FareIndGroups", "FareIndividualBySex", "AgePw2"  
"Pclass", "KNNAgesScaled", "FareIndividualBySexScaled"  
"SibSp", "FareIndividualBySex", "AgePw3", "PclassScaled"  

```{r pruebas_analisis_FS_BS_rlm, size="small", echo=FALSE, warning=FALSE}

trainRlm <- muestrascaledumy[1:891,-c(4,9,11,13,14,20)]
#trainRlm <- muestra[1:891,-c(4,9,11,13,14,20,54)]
...
```
```{r pruebas_analisis_FS_BS_rlm2, size="small",include=FALSE, echo=FALSE, warning=FALSE}
# Running the selection algorithm
res <- selectFeatures("classif.rpart", trainRlm.task, rdesc, control = ctrl)
```
```{r pruebas_analisis_FS_BS_rlm3, size="small", echo=FALSE, warning=FALSE}
...
Variables recomendadas por este método:  
"FareIndividualBySex", "Titulo", "FareIndividual", "KNNAges", "Sex"  
FareIndividualBySex, Titulo, KNNAgesScaled  

A partir de la lista de importancia de las variables, obtenemos las más importantes que no estén relacionadas entre si:  
"FareIndividualBySex","Titulo","KNNAges",'PclassScaled','FamilyIDTKGrouped','Embarked','LoneWolfsDummy','youngTravelAl','IsMother','tamanoticket'  


```{r pruebas_analisis_rfe, size="small", echo=FALSE, warning=FALSE}
#quitamos el 37 por el limite máximo de categorias
#trainRFE <- muestrascaledumy[1:891,-c(4,9,11,13,14,20)]
trainRFE <- muestra[1:891,-c(4,9,11,13,14,54)]
trainRFE$Survived<-as.factor(trainRFE$Survived)
...
```{r pruebas_analisis_rcaret, size="small", echo=FALSE, include=FALSE}
...
Variables recomendadas por este método:   
'FareIndividualBySex','Titulo','Mr','KNNAges','FIBSG14','FamilyIDTKGrouped','Pclass','tamanoticket','FIBSG51','Mrs','Miss'  
'FareIndividualBySex','Titulo','KNNAges','FamilyIDTKGrouped','PclassFactor','tamanoticket','Embarked','LoneWolfsDummy','IsMotherDummy','youngTravelAlDummy'  
'FareIndividualBySex','Titulo','ageGroupsByCut','FamilyIDTKGrouped','PclassFactor','tamanoticket','Embarked','LoneWolfsDummy','IsMotherDummy','youngTravelAlDummy'  
...
```{r pruebas_analisis_RForest, size="small", echo=FALSE, warning=FALSE}
#quitamos el 37 por el limite máximo de categorias
#trainRF <- muestrascaledumy[1:891,-c(4,9,11,13,14)]
#testRF <- muestrascaledumy[892:1309,-c(4,9,11,13,14)]
trainRF <- muestra[1:891,-c(4,9,11,13,14,54)]
testRF <- muestrascaledumy[892:1309,-c(4,9,11,13,14,54)]
trainRF$FamilyIDTKGrouped<-as.factor(trainRF$FamilyIDTKGrouped)
...
'PassengerId','Survived',"FamilyIDTKGrouped","Titulo","FareIndividual","IsMother","LoneWolfs","youngTravelAl","Age","Embarked","Pclass" - el resultado obtenido lo subimos a kaggle y nos dió: 0.779 

'FareIndividualBySex','Titulo','ageGroupsByCut','FamilyIDTKGrouped','PclassFactor','tamanoticket','Embarked','LoneWolfsDummy','IsMotherDummy','youngTravelAlDummy' - en kaggle obtuvimos un 0.794

```{r pruebas_predict_rf, size="small", echo=FALSE, warning=FALSE}
# Partimos el dataset en los conjuntos iniciales de train y test.
#train2 <- muestra[1:891,c('PassengerId','Survived','Pclass','Sex','Age','SibSp','Parch','Fare','Embarked','Titulo','FamSize')] #82%
#test2 <- muestra[892:1309,c('PassengerId','Survived','Pclass','Sex','Age','SibSp','Parch','Fare','Embarked','Titulo','FamSize')]

#train2 <- muestra[1:891,c('PassengerId','Survived',"FareIndividualBySex","IsMother","LoneWolfs","TicketFreq","ageGroupsByCut","Embarked","Pclass")] #83.5%
#test2 <- muestra[892:1309,c('PassengerId','Survived',"FareIndividualBySex","IsMother","LoneWolfs","TicketFreq","ageGroupsByCut","Embarked","Pclass")]

train2 <- muestra[1:891,c('PassengerId','Survived',"FamilyIDTKGrouped","Titulo","FareIndividual","IsMother","LoneWolfs","youngTravelAl","Age","Embarked","Pclass")] #83.58%
test2 <- muestra[892:1309,c('PassengerId','Survived',"FamilyIDTKGrouped","Titulo","FareIndividual","IsMother","LoneWolfs","youngTravelAl","Age","Embarked","Pclass")]

#train2 <- muestra[1:891,c('PassengerId','Survived',"FareIndividualBySexGroups","IsMother","LoneWolfs","TicketFreq","ageGroupsByCut","Embarked","Pclass",'FarePw2')]  #82.8
#test2 <- muestra[892:1309,c('PassengerId','Survived',"FareIndividualBySexGroups","IsMother","LoneWolfs","TicketFreq","ageGroupsByCut","Embarked","Pclass",'FarePw2')]

#train2 <- muestra[1:891,c('PassengerId','Survived','FareIndividualBySex','Titulo','KNNAges','FamilyIDTKGrouped','PclassFactor','tamanoticket','Embarked','LoneWolfsDummy','IsMotherDummy','youngTravelAlDummy')] #82%-80%
#test2 <- muestra[892:1309,c('PassengerId','Survived','FareIndividualBySex','Titulo','KNNAges','FamilyIDTKGrouped','PclassFactor','tamanoticket','Embarked','LoneWolfsDummy','IsMotherDummy','youngTravelAlDummy')]

#train2 <- muestrascaledumy[1:891,c('PassengerId','Survived','FareIndividualBySex','Titulo','Mr','KNNAges','FIBSG14','FamilyIDTKGrouped','Pclass','tamanoticket','FIBSG51','Mrs','Miss')]
#test2 <- muestrascaledumy[892:1309,c('PassengerId','Survived','FareIndividualBySex','Titulo','Mr','KNNAges','FIBSG14','FamilyIDTKGrouped','Pclass','tamanoticket','FIBSG51','Mrs','Miss')]

# train2<-BkpTrainFamd11c100[1:891,] #82%
# test2 <- BkpTrainFamd11c100[892:1309,]
...
Prediction <- predict(fit, test2)
submittest3 <- data.frame(PassengerId = test2$PassengerId, Survived = Prediction)
#pasamos dataset a fichero
#write.csv(submittest3, file = "Pred_Rforest.csv", row.names = FALSE)
...
#                data = train4[,-1], controls=cforest_unbiased(ntree=2000, mtry=3)) 
# # hacemos la predicción y la guardamos en formato de envio 
...
Este método obtiene los valores de Survived en funcion de sus vecinos más proximos que encuentra a partir de la distáncia euclídea.
Para usar este método escalamos todas las variables cuantitativas y pasamos todas las variables cualitativas a dummy, creando una variable numérica por cada uno de los factores de las variables.

El resultado obtenido por este método fue subido a kaggle y dió: 0.732

```{r pruebas_predict_knn, size="small", echo=FALSE, warning=FALSE}

# # Predicciones por KNN
# ## set the seed to make your partition reproducible
# set.seed(123)
# #creamos un conjunto de entrenamiento y test propios a partir de train
# train3<-muestra[1:891,]
...
# 
# #dfinputaciones<-kNN(train3, variable=c("Survived"), dist_var=c("FareIndividualBySexScaled","IsMotherDummy","LoneWolfsDummy","TicketFreqScaled","PclassScaled",'FamSizeScaled'),k=5) #079
# 
# #0.835
...
# 
# dfinputaciones<-kNN(train3scaledumy, variable=c("Survived"), dist_var=c('FareIndividualBySex','Titulo','KNNAges','FamilyIDTKGrouped','PclassFactor','tamanoticket','Embarked','LoneWolfsDummy','IsMotherDummy','youngTravelAlDummy'),k=5)
# 
# #dfinputaciones<-kNN(train3, variable=c("Survived"), dist_var=c("FareIndividualBySex","Titulo","KNNAges",'PclassScaled','FamilyIDTKGrouped','Embarked','LoneWolfsDummy','youngTravelAl','IsMother','tamanoticket'),k=25)
# 
# #0.835
...
# Almacenamos los valores predichos de Survived para enviar
KNNsurvived<-dfinputSurvived$Survived
submittest3cf <- data.frame(PassengerId = train3scaledumy[892:1309,]$PassengerId, Survived = KNNsurvived)
#escribimos fichero
#write.csv(submittest3cf, file = "Pred_knn.csv", row.names = FALSE)
...
```{r pruebas_predict_rl, size="small", echo=FALSE, warning=FALSE}

trainRL<-muestra
#ajustamos los valores de referencia en los factores
trainRL$Sex <- relevel(trainRL$Sex, ref = 'male')
trainRL$Embarked <- relevel(trainRL$Embarked, ref = 'S')
trainRL$IsMother <- relevel(trainRL$IsMother, ref = 'FALSE')
trainRL$LoneWolfs <- relevel(trainRL$LoneWolfs, ref = 'TRUE')
trainRL$Titulo <- relevel(trainRL$Titulo, ref = 'Mr')
trainRL$FamilyIDTKGrouped <- relevel(trainRL$FamilyIDTKGrouped, ref = 'Alone')
trainRL$Survived<-as.factor(trainRL$Survived)
# Partimos el dataset en los conjuntos iniciales de train y test.
train2 <- trainRL[1:891,]
test2 <- trainRL[892:1309,]
...
# 
# #corregimos dataset
# train2$IsMother <- as.factor(train2$IsMother)
# train2$LoneWolfs <- as.factor(train2$LoneWolfs)
# #ajustamos los valores de referencia en los factores
# train2$Sex <- relevel(train2$Sex, ref = 'male')
# train2$Embarked <- relevel(train2$Embarked, ref = 'S')
# train2$IsMother <- relevel(train2$IsMother, ref = 'FALSE')
# train2$LoneWolfs <- relevel(train2$LoneWolfs, ref = 'TRUE')
# train2$Titulo <- relevel(train2$Titulo, ref = 'Mr')
# train2$TicketFreq <- relevel(train2$TicketFreq, ref = 'Alone')
...
#mymod3 <- glm(y ~ x1+x2+x12+x10+x11+x17+x15, binomial, data=)

#mymod3 <- glm(Survived ~ Pclass+SibSp+TicketFreq+Titulo+FamilyIDTKGrouped+ageGroupsByCut+FareIndGroups, binomial, data=train2) #838
#mymod3 <- glm(Survived ~ PclassScaled+SibSpScaled+TicketFreqScaled+Titulo+FamilyIDTKGrouped+ageGroupsByCut+FareIndGroups, binomial, data=train2)
#mymod3<-glm(Survived ~ Titulo+FareIndividual+Pclass+FamilyIDTKGrouped, data = train2 ,binomial)
#mymod3<-glm(Survived ~ Titulo+FareIndividualScaled+PclassScaled+FamilyIDTKGrouped, data = train2 ,binomial)
#mymod3<-glm(Survived ~ Pclass+Titulo+FamSize+FareIndGroups+FareIndividualBySex+AgePw2, data = train2 ,binomial) #0.839 - kaggle 0.76
mymod3<-glm(Survived ~ FareIndividualBySexScaled+IsMotherDummy+LoneWolfsDummy+TicketFreqScaled+ageGroupsByCut+Embarked+PclassScaled, data = train2 ,binomial)
summary(mymod3)
#table(y) 
...
## vamos a ver los resultados del modelo para ver que tal lo hace
probabilidades <- predict(mymod3, type="response", data=train2) 
# si la predicción supera el 50% se asigna a peso bajo
predicciones <- ifelse(probabilidades > 0.5, "Sobrevive", "No Sobrevive")
...
# ## para envio
# probabilidadestest <- predict(mymod3, type="response", newdata=test2) 
# prediccionesOptimizadasEnvio <- ifelse(probabilidadestest > optCutOff, 1, 0)
# #preparamos fichero de envío
# submittestRL <- data.frame(PassengerId = test2$PassengerId, Survived = prediccionesOptimizadasEnvio)
# #pasamos dataset a fichero
# write.csv(submittestRL, file = "Pred_RLogistic.csv", row.names = FALSE)
...
likelihoodRatio
model_null = glm(Survived~1, binomial, data=train2) #creamos una modelo solo con la constante para comparar el LR, parece que lo hace ya de serie si no se pasan 2 modelos
...
hl
#This gives p=0.047, indicating some evidence of poor fit. This is good, since here we know the model is indeed correctly specified. We can also obtain a table of observed vs expected, from our hl object:
...
Después optamos por usar alguno de los conjuntos de variables selecionados en el análisis y obtuvimos mejores resultados, llegando a 0.784.

```{r pruebas_predict_caret, size="small", echo=FALSE}
...
# 
# nnModel <- caret::train(Survived ~ FareIndividualBySex+IsMother+LoneWolfs+TicketFreq+ageGroupsByCut+Embarked+Pclass, data = training, trControl = train_control,
...
No se nos ocurre como representar los resultados obtenidos, ya que no sabemos nada más que el porcentaje de acierto que nos facilita Kaggle.
Hemos usado las gráficas en las diferentes fases del proceso.
...
Sobra decir que por toda la literatura que hay acerca de esta competición, hemos visto métodos de predicción que obtienen mejores resultados que los nuestros, pero decidimos probar por nuestra cuenta para ver hasta donde podíamos llegar. Inicialmente nos ha costado que todo simplemente funcionase, pero hemos aprendido bastante y poco a poco hemos mejorado nuestros conocimientos de las técnicas que hemos usado, lo que se ha ido reflejando en una mejoría en el porcentaje de éxito.  
Si dispusiesemos de algo más de tiempo supongo que podríamos haber superado la barrera del 80%, probablemente lo hagamos próximamente, pero ya no será como parte de la práctica.
