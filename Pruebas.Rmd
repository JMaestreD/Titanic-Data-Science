...
```{r pruebas_mrzs_Pclass, size="small"}
...
Escaladas:
'AgePw2','FarePw2','AgePw3','FarePw3','PclassScaled','AgeScaled','SibSpScaled','ParchScaled','FareScaled','tamanoticketScaled','TicketFreqScaled','FamSizeScaled','FareIndividualScaled','AgesScaled','KNNAgesScaled','RegresionAgesScaled','FareIndividualBySexScaled'

Factorizadas:
"PclassFactor",'SurvivedFactor'

Dummy:
'SexDummy','LoneWolfsDummy','IsMotherDummy','youngTravelAlDummy'


grupos de variables relacionadas a priori:
...
Vamos a analizar la asociacion entre variables de nuestros grupos y entre algunos de ellos:
...
res.famd <- FAMD(noNaGroup, ncp = 18,graph = F)
...
trainFAMD <- data.frame(muestra[,1:2],res.famd$ind$coord[1:nrow(muestra),1:18])
...
train2 <- muestra[1:891,c('PassengerId','Survived','FareIndividualBySex','Titulo','ageGroupsByCut','FamilyIDTKGrouped','PclassFactor','tamanoticket','Embarked','LoneWolfsDummy','IsMotherDummy','youngTravelAlDummy')] #Kaggle0.79
test2 <- muestra[892:1309,c('PassengerId','Survived','FareIndividualBySex','Titulo','ageGroupsByCut','FamilyIDTKGrouped','PclassFactor','tamanoticket','Embarked','LoneWolfsDummy','IsMotherDummy','youngTravelAlDummy')]

# train2 <- muestra[1:891,c('PassengerId','Survived','FareIndividualBySex','Titulo','KNNAges','FamilyIDTKGrouped','PclassFactor','tamanoticket','Embarked','LoneWolfsDummy','IsMotherDummy','youngTravelAlDummy')] #82%-80% Kaggle0.77
...
submittest3cf <- data.frame(PassengerId = test4$PassengerId, Survived = Prediction)
#escribimos fichero
write.csv(submittest3cf, file = "Pred_ciforest.csv", row.names = FALSE)
#Vamos a comparar el resultado con las observaciones de la variable y construir una matriz de acierto
